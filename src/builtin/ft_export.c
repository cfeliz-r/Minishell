...
static t_list_env *copy_list(t_list_env *original)
{
    ...
    t_list_env *new_list = create_node(original->envp_content);
    if (!new_list)
        return NULL;

    t_list_env *current_original = original->next;
    t_list_env *current_new = new_list;
...
static void update_variable_content(t_list_env *current, const char *key, const char *value)
{
    ...
    char *new_content = malloc(total_len);
    if (!new_content)
        return;
