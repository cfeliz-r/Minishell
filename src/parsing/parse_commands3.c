...
/*   Updated: 2024/08/15 09:43:03 by manufern         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *handle_variable_expansion(const char *command, t_parse_context *ctx, t_list_env *envp) {
    char key[256];
    size_t k = 0;
    char *value;

    while (command[ctx->i] && (ft_isalnum(command[ctx->i]) || command[ctx->i] == '_') && k < sizeof(key) - 1)
        key[k++] = command[(ctx->i)++];
    key[k] = '\0';
...
static char *process_char(const char *command, t_parse_context *ctx, t_list_env *envp) {
    if (command[ctx->i] == '"' && ctx->in_single_quotes == 0) {
        // Entra o sale de comillas dobles
        ctx->in_double_quotes = !ctx->in_double_quotes;
        ctx->result = append_char(ctx->result, command[ctx->i], &(ctx->j), &(ctx->buffer_size)); // Mantener comilla doble
        ctx->i++;
    } else if (command[ctx->i] == '\'' && ctx->in_double_quotes == 0) {
        // Entra o sale de comillas simples
        ctx->in_single_quotes = !ctx->in_single_quotes;
        ctx->result = append_char(ctx->result, command[ctx->i], &(ctx->j), &(ctx->buffer_size)); // Mantener comilla simple
        ctx->i++;
    } else if (command[ctx->i] == '$' && ctx->in_single_quotes == 0) {
        // Expandir variables solo fuera de comillas simples
        ctx->result = handle_dollar_sign(command, ctx, envp);
    } else {
        // Procesar cualquier otro carácter
...
char *interpret_command(const char *command, t_list_env *envp) {
    ...
    // Asegurarse de que todas las comillas estén cerradas
